    ...
    <div class="container" style="">
        <div class="row rounded-3 mx-5 mb-3" style="">
            <div class="row mb-2" style="">
                <a class="fs-4 fw-bolder" href='/genre/{{ $genre->id }}'>{{ $genre->name }}</a>
            </div>
            <div class="row d-flex justify-content-center">
                @foreach ($movies as $item)
                    <div class="card border-0 p-2 mx-1 mb-4" style="width: 24%">
                        <img src="{{ $item->photo }}" class="card-img-top" alt="" style="height: 100%">
                        <h5 class="card-title text-center p-1">{{ $item->title }}</h5>
                        <a type="button" class="btn btn-danger" href="/movie/{{ $item->id }}">SEE MOVIE</a>
                    </div>
                @endforeach
            </div>
